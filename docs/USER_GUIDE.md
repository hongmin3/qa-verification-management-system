...
운영 서버는 그 폴더를 **읽기 전용으로 마운트**해서 본다. 마운트가 붙어 있으면 서버 화면의
"지금 수집" 버튼으로 실행되고 주간 자동 수집도 서버가 한다
([설정 절차](POST_DEPLOY_TESTS.md#2-제품-지식-폴더-수집--서버에서-되는지가-관건)).

마운트가 없거나 끊긴 상태에서는 버튼이 숨고 화면이 CLI 절차를 안내한다. 그때는 폴더가
있는 PC에서 실행한다.
